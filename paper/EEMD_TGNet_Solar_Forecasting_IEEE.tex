...
\author{
\IEEEauthorblockN{Huy-Toan Nguyen\IEEEauthorrefmark{1}, 
Tuan-Anh Pham\IEEEauthorrefmark{1}, 
Hao-Nguyen Nguyen\IEEEauthorrefmark{1}, 
Van-Huy Tran\IEEEauthorrefmark{1}, 
Van-Lam Tran\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}\textit{UData.AI Research Lab} \\
\textit{UData Technology JSC}\\
Hanoi, Vietnam \\
\{huy.toan, tuan.anh, hao.nguyen, van.huy, van.lam\}@udata.ai}
}
...
\subsection{CNN-Based Hybrid Architectures}
The integration of Convolutional Neural Networks (CNNs) with recurrent architectures has proven highly effective for solar forecasting. Boucetta et al. \cite{b3} proposed a CNN-LSTM hybrid using Variational Mode Decomposition (VMD), achieving significant improvements through signal preprocessing. Recent 2024 studies have extended this approach with CNN-BiLSTM and CNN-GRU variants, where CNN-BiGRU models demonstrated up to 85.71\% error reduction compared to standalone BiLSTM approaches. Advanced CNN-LSTM-RF hybrid models have achieved R² scores of 92\% with RMSE of 0.07 kW, showcasing the power of multi-architecture fusion.

\subsection{Transformer and Attention-Based Models}
The application of transformer architectures to solar forecasting has gained significant momentum. Wu et al. \cite{b6} introduced Autoformer with decomposition-based auto-correlation mechanisms for long-term forecasting. Recent developments include Transformer-Infused RNN (TIR) models that integrate BiLSTM encoders with GRU decoders, enhanced with attention mechanisms and positional encoding. Vision Transformer (ViT) variants have achieved 10.7-18.5\% skill score improvements for ultra-short-term PV forecasting using sky images.
...
\subsection{Edge Computing and Model Optimization}
Edge intelligence for solar energy management has become increasingly important for real-world deployment \cite{b2}. Recent research focuses on model pruning, quantization, and efficient architectures suitable for resource-constrained IoT devices. ResNet-based architectures address vanishing gradient problems while maintaining computational efficiency for photovoltaic forecasting applications.
...
\begin{itemize}
    \item \textbf{Architectural Evolution:} Progression from basic RNNs to sophisticated CNN-hybrid and transformer-based models with attention mechanisms.
    \item \textbf{Signal Processing Integration:} Decomposition methods (EEMD, VMD, CEEMDAN) remain critical for handling non-stationary solar data effectively.
    \item \textbf{Multi-modal Fusion:} Integration of meteorological data, sky imagery, and temporal features through advanced attention mechanisms.
    \item \textbf{Edge Optimization:} Growing emphasis on model efficiency and deployment capabilities for real-world IoT applications.
    \item \textbf{Ensemble Superiority:} Consistent evidence that ensemble and hybrid approaches outperform single-model architectures across diverse datasets.
\end{itemize}
...
\textbf{Basic RNN Models:} LSTM, GRU, BiLSTM, and MLP serving as fundamental baselines.

\textbf{CNN-Hybrid Architectures:} CNN-LSTM, CNN-BiLSTM, CNN-GRU, and CNN-BiGRU representing spatial-temporal feature extraction approaches.

\textbf{Advanced Hybrid Models:} CNN-LSTM-RF, CNN-SLSTM (Stacked LSTM), and ResNet-LSTM incorporating ensemble and residual learning techniques.

\textbf{Transformer-Based Models:} Transformer, Informer, Autoformer, and PatchTST representing the latest advances in attention-based time series forecasting.

\textbf{Vision-Based Models:} ViT-GRU, ConvLSTM, and Attention-LSTM integrating visual processing capabilities with temporal modeling.

\textbf{Ensemble Models:} Wavelet-BiLSTM, EEMD-BiLSTM, TGNet, and our proposed EEMD-TGNet leveraging signal decomposition and ensemble strategies.
...
\subsection{Comprehensive Model Category Analysis}
...
\subsection{Advanced Model Performance Analysis}
...
\begin{table}[htbp]
\centering
\caption{Performance comparison of top-performing models across all datasets (MSE values). Best results in bold.}
\label{tab:top_models}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{Udata} & \textbf{GEFCom2014} & \textbf{Chinese Grid} \\ \midrule
\textbf{EEMD-TGNet} & \textbf{0.167} & \textbf{0.113} & \textbf{0.098} \\
...
LSTM (Baseline) & 0.289 & 0.196 & 0.178 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/advanced_performance_comparison.png}
    \caption{Comprehensive performance comparison across 22 state-of-the-art models and all datasets. EEMD-TGNet (highlighted with black borders) consistently achieves superior performance across all metrics.}
    \label{fig:advanced_performance_comparison}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/model_category_analysis.png}
    \caption{Model category performance analysis showing ensemble models achieve the best average performance across all datasets}
    \label{fig:category_analysis}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/improvement_analysis.png}
    \caption{Performance improvement analysis over LSTM baseline demonstrating EEMD-TGNet's superior advancement}
    \label{fig:improvement_analysis}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/performance_scatter_matrix.png}
    \caption{Performance scatter matrix revealing model relationships across metrics with EEMD-TGNet (red circles) consistently in optimal regions}
    \label{fig:scatter_matrix}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/model_ranking_heatmap.png}
    \caption{Model ranking heatmap across datasets with EEMD-TGNet (red borders) achieving top rankings consistently}
...
\bibitem{b1} T. H. T. Nguyen, Q. B. Phan, V. N. N. Nguyen, and H. M. Pham, "Day-ahead electricity load forecasting based on hybrid model of EEMD and Bidirectional LSTM," in \textit{Proc. 5th Int. Conf. Future Networks Distrib. Syst. (ICFNDS)}, Dubai, UAE, Dec. 2021, pp. 1--8.
...
\bibitem{b5} W. Li, H. Yang, J. Zhao, W. Liu, and Y. Zhang, "Multi-step wind speed prediction based on a hybrid TCN-GRU model," in \textit{Proc. IEEE Sustain. Power Energy Conf. (iSPEC)}, Nanjing, China, Nov. 2021, pp. 1096--1101.

\bibitem{b6} H. Wu, J. Xu, J. Wang, and M. Long, "Autoformer: Decomposition transformers with auto-correlation for long-term series forecasting," in \textit{Proc. Adv. Neural Inf. Process. Syst. (NeurIPS)}, vol. 34, 2021, pp. 22419--22430.
...
\bibitem{b9} Y. Nie, N. H. Nguyen, P. Sinthong, and J. Kalagnanam, "A time series is worth 64 words: Long-term forecasting with transformers," in \textit{Proc. Int. Conf. Learn. Represent. (ICLR)}, 2023, pp. 1--16.

\bibitem{b10} A. Zeng, M. Chen, L. Zhang, and Q. Xu, "Are transformers effective for time series forecasting?" in \textit{Proc. AAAI Conf. Artif. Intell.}, vol. 37, Washington, DC, USA, Feb. 2023, pp. 11121--11128.

\bibitem{b11} H. Zhou, S. Zhang, J. Peng, S. Zhang, J. Li, H. Xiong, and W. Zhang, "Informer: Beyond efficient transformer for long sequence time-series forecasting," in \textit{Proc. AAAI Conf. Artif. Intell.}, vol. 35, no. 12, May 2021, pp. 11106--11115.

\bibitem{b12} Y. Liu, H. Wu, J. Wang, and M. Long, "Non-stationary transformers: Exploring the stationarity in time series forecasting," in \textit{Proc. Adv. Neural Inf. Process. Syst. (NeurIPS)}, vol. 35, 2022, pp. 9881--9893.
\end{thebibliography}
